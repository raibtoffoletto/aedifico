            ...
            if (file.isDirectory ()) {
                fs.readdirSync (filepath).forEach (f => {
                    ...
                    if (subdir) {
                        ...
                        if (!file_mime) {file_mime = "text/plain";}
        ...
        files.forEach (f =>  {
            if (f.name !== 'menu.md') {
                ...
                let page = f;
                    ...
                    page.date = f.modify.getFullYear () + '/' +
                                f.modify.getMonth () + '/' +
                                f.modify.getDate () + ' at ' +
                                f.modify.getHours () + ':' +
                                f.modify.getMinutes ();
...
var getPosts = function () {
    let posts = {path: path.join (__dirname, '../repo.git/public/content/posts')};
        posts.list = [];
    let files = getFileList (posts.path, true, true);
        files.forEach (f =>  {
            ...
            let post = f;
                ...
                post.date = f.modify.getFullYear () + '/' +
                            f.modify.getMonth () + '/' +
                            f.modify.getDate () + ' at ' +
                            f.modify.getHours () + ':' +
                            f.modify.getMinutes ();
...
var getMenu = function () {
    ...
    getPages ().list.forEach (e => {
        ...
        if ( e_link == -1 && e.name != "index.md") {
            pages.push ({item: e.title, link: '/' + e.name.slice (0, e.name.length - 3)});
        }
    });
...
var updateMenu = function (new_order) {
    ...
    try {
        fs.writeFileSync (path.join (__dirname, '../repo.git/public/content/menu.md'), updated_menu);
