...
var checkRepo = function () {
    ...
    new Promise (function (resolve, reject) {
        ...
        if (fs.readdirSync (repo_path).length == 0) {
            ...
            git.Clone (repo_remote, repo_path, {checkoutBranch: 'master'}).then (function () {
...
var getServiceStatus = async function () {
    try {
        let aedifico = exec ('systemctl --lines 0 status aedifico.service').toString ();
        let aedifico_preview = exec ('systemctl --lines 0 status aedifico-preview.service').toString ();
...
var commitToPreview = async function () {
    try {
        ...
        git.Repository.open (repo_path).then (function (got_repo) {
            repo = got_repo;
            ...
            repo.createCommit ("HEAD", author, author,
                                "Sprintplank commit ",
                                tree_oid, [head_commit]);
        }).then (function () {
